...
{	
    ...
    public function run()
    {	
    	$programm = Programm::where('slug','=','r.one_start')->first();
...
Перед выполнением всех упражнений сразу или по отдельности рекомендуется сделать общую разминку, а после окончания выполнения тренировки – растяжку.', 'feed' => 'Начинаем неспешно , но продуктивно!) Кушаем в первый день маленькими порциями, стараемся есть больше белка (яйца, творог, сыр). Обедаем и ужинаем очень легко. Из мяса желательно кролик, курица или рыба. От гарнира желательно отказаться, заменить растительным белком (зелёная фасоль, бобовые). Перекусываем после тренировки молочным коктейлем или орехами.','interest' => 1, 'difficult' => 2, 'lead_time' => '< 20 мин', 'free_day' => 1, 'stages' => [
				...
				['day' => 2, 'status' => 1, 'description' => '"Вспомогательная тренировка для укрепления всего организма. Развивается общая выносливость и помощь в ускорении обмена веществ. Каждое упражнение выполняется в течение 1 минуты, отдых между переходом на другое упражнение 10-15 сек. Выполняется 3 круга."', 'feed' => 'Перед тренировкой, за час - скушайте кусок чёрного зернового хлеба с чаем/водой. Это даст вас углеводный запас и энергию для Старта!) После тренировки (через мин 15)  желательно выпить молочный коктейль/фрукт или же выпить свежевыжатый сок . Все это пополнит запас гликогена быстро и безвредно для фигуры', 'lead_time' => '< 20 мин','interest' => 0, 'difficult' => 1, 'free_day' => 1, 'stages' => [
    		...
    		foreach ( $programm_days_array as $programm_day_row ) {
	        	...
	        	if (isset($programm_day_row['free_day'])) {
	        		$programm_day->free_day = $programm_day_row['free_day'];
	        	}
